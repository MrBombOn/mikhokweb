...
| Elem | Leírás |
|------|--------|
| **Projekt** | PTE MIK HÖK hallgatói önkormányzat webes felülete – landing, modulok (naptár, kalkulátor, galéria, útmutatók, hírek, about, office), mock admin, későbbi valós auth és API. |
| **Stack** | Next.js **15** (App Router), React **19**, TypeScript, ESLint 9 (flat config), `globals.css` alapú design tokenek. |
| **Adat / API** | A legtöbb tartalom **kliensoldali mock** és `lib/content.ts` statikus tömbjei; `app/api/*` végpontok jelenleg **501 Not Implemented** stubok. |
...
| `npm run lint` | `next lint` (Next 16 előtti útvonal; deprecation figyelmeztetés lehetséges) |
...
| `app/globals.css` | Design tokenek, light/dark (`data-theme`), komponensosztályok. |
...
| Elem | Megjegyzés |
|------|------------|
| `prisma/`, `content/`, `styles/`, `tests/`, `public/` | Spec szerinti mappák; tartalom fokozatosan töltődik. |
...
- **Footer**: szerver komponens; belső navigációs `Link`-ek (nincs külső repó link a felületen). A GitHub URL a repó dokumentációjában van: [`docs/repository.md`](./docs/repository.md), [`README.md`](./README.md).
...
| Mező | Érték |
|------|--------|
| **Dokumentum** | `DOCUMENTATION_PROJECT_WORKFLOW.md` |
