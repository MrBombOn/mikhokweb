/**
 * @file REST: About tag – GET / PATCH / DELETE (soft delete).
 */
...
import {
  ...
  parseAboutMemberId,
  patchAboutMember,
  softDeleteAboutMember,
} from '@/features/about/server';
import { patchAboutMemberSchema } from '@/lib/validation/about';
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const id = parseAboutMemberId(raw);
  if (id == null) {
    return NextResponse.json({ error: 'Érvénytelen azonosító.' }, { status: 400 });
  }

  const user = await getCurrentUser();
  const result = await getAboutMemberDto(id, user?.role);
  if (!result.ok) {
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  const result = await softDeleteAboutMember(id);
  if (!result.ok) {
    return NextResponse.json({ error: 'Nem található.' }, { status: result.status });
  }
  await writeAudit({
    actor: user,
    action: 'delete_about_member',
    entityType: 'about_member',
    entityId: String(id),
  });
